#!/bin/bash
# ══════════════════════════════════════════════════
# LinhIQ — VPS Initial Setup Script
# ══════════════════════════════════════════════════
...
DEPLOY_USER="deploy"
APP_DIR="/home/$DEPLOY_USER/linhiq"

echo "═══════════════════════════════════════"
echo "🔧 LinhIQ VPS Initial Setup"
echo "═══════════════════════════════════════"
...
su - $DEPLOY_USER -c "
  mkdir -p ~/linhiq ~/logs ~/backups/db ~/scripts
"
...
docker exec linhiq-postgres pg_dump -U linhiq linhiq | gzip > "$BACKUP_DIR/linhiq-$TIMESTAMP.sql.gz"

# Remove old backups
find "$BACKUP_DIR" -name "*.sql.gz" -mtime +$RETENTION_DAYS -delete

echo "✅ Backup complete: linhiq-$TIMESTAMP.sql.gz ($(du -h "$BACKUP_DIR/linhiq-$TIMESTAMP.sql.gz" | cut -f1))"
BACKUP
...
cat > /etc/logrotate.d/linhiq << 'LOGROTATE'
...
echo "  2. Clone your repo:    git clone <your-repo> ~/linhiq"
echo "  3. Copy .env.prod:     cp .env.prod.example .env.prod"
echo "  4. Edit .env.prod:     nano .env.prod"
echo "  5. Initial SSL setup:  cd ~/linhiq && bash deploy/ssl-init.sh"
